...
unsigned char String::resize(unsigned int size) {
    ...
    //ESP_LOGI(TAG, "before: c=%d, l=%d, target=%d, new=%d", capacity, len, size, newSize);
	...
	if(newbuffer) {
		...
		len = size;
		buffer = newbuffer;
	    //ESP_LOGI(TAG, "after: c=%d, l=%d, target=%d, new=%d, ptr=%p", capacity, len, size, newSize, newbuffer);
		return 1;
	}
	return 0;
