...
bool Config::Read() {
	...
	if (nvs_flash_init() != ESP_OK)
		return false;
	if (nvs_open("Esp Config", NVS_READONLY, &h) != ESP_OK)
		return false;
	...
	ReadString(h, "DeviceId", msDeviceId);
...
bool Config::Write()
{
	...
	if (nvs_flash_init() != ESP_OK)
		return false;
	if (nvs_open("Esp Config", NVS_READWRITE, &h) != ESP_OK)
		return false;
	...
	if (!WriteString(h, "DeviceId", msDeviceId))
		return nvs_close(h), false;
